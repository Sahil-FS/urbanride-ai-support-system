...
export function MessageBubble({
  ...
  // A final answer = no pills + no call button from API + not escalation + not welcome
  const isFinalAnswer =
    !isUser &&
  ...
  // ── Call button — direct call on mobile, alert on desktop ────────────────
  const triggerCall = () => {
    onCallClick(); // generates ticket
  ...
  // Call button shown from API (escalation / SOS / emergency messages)
  ...
  // Call button shown after "No, still need help"
  // This is the key piece — customer not satisfied → call appears here
  ...
  return (
    <div className={`bubble-row ${isUser ? 'bubble-row--user' : 'bubble-row--bot'}`}>
      ...
      <div className="bubble-content">
        ...
        {/* Sub-option pills — disappear after one click */}
        {!isUser &&
          message.subOptions &&
        ...
        {/* Call button from API (emergency / escalation messages) */}
        {!isUser && message.showCallButton && renderApiCallButton()}

        {/* Satisfaction check — only on final answer messages */}
        ...
        {/* Customer NOT satisfied → call button appears here */}
        {isFinalAnswer &&
      ...
      {isUser && (
        <div className="bubble-avatar bubble-avatar--user">AK</div>
      )}
    </div>
  );
}
